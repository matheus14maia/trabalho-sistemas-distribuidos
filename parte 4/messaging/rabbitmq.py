...
class UpdatePublisher:
    """
    Publica comandos já aplicados pelo líder para o exchange fanout.
    """
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                self._logger.info(f"Publicador conectado ao RabbitMQ (exchange {self._exchange_name})")
                return
            except Exception:
...
class UpdateConsumer:
    """
    Consome comandos do exchange fanout (seguidores).
    """
    ...
    async def _handle_message(self, message: aio_pika.IncomingMessage) -> None:
        async with message.process(ignore_processed=True):
            ...
            try:
                ...
                self._logger.exception("Erro ao processar comando recebido do exchange")
