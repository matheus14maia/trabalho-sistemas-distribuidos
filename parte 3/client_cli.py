...
HELP_TEXT = """Comandos:
  SET <chave> <valor>    -> envia comando SET via RPC (gRPC JSON)
  GET <chave>            -> envia comando GET via RPC e mostra valor local retornado pelo nó
  PRINT                  -> envia comando PRINT via RPC e mostra snapshot local retornado
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cliente CLI para o serviço RPC do KV Store (parte 3).")
    parser.add_argument(
        "--target", default="localhost:50061", help="host:porta do serviço gRPC (ex.: node1:50061)"
    )
